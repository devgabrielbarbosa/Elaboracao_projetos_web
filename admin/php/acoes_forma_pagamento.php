...
switch($acao) {
    case 'adicionar':
        ...
        if(!$nome) { echo json_encode(['sucesso'=>false,'mensagem'=>'Nome vazio']); exit; }
        $stmt = $pdo->prepare("INSERT INTO formas_pagamento (nome, admin_id) VALUES (:nome, :admin_id)");
        $stmt->execute([':nome'=>$nome, ':admin_id'=>$admin_id]);
        echo json_encode(['sucesso'=>true,'mensagem'=>'Forma adicionada!']);
        break;

    case 'excluir':
        if(!$id) { echo json_encode(['sucesso'=>false,'mensagem'=>'ID inválido']); exit; }
    ...
    case 'toggle':
        ...
        if(!$forma) { echo json_encode(['sucesso'=>false,'mensagem'=>'Registro não encontrado']); exit; }
        $novo = $forma['ativo'] ? 0 : 1;
        $stmt = $pdo->prepare("UPDATE formas_pagamento SET ativo=:novo WHERE id=:id AND admin_id=:admin_id");
        $stmt->execute([':novo'=>$novo, ':id'=>$id, ':admin_id'=>$admin_id]);

...
try {
    // ---------- LISTAR ----------
    if (!$acao || $acao === 'listar') {
        $stmt = $pdo->prepare("SELECT id, nome, ativo FROM categorias_produtos_lojas WHERE loja_id = :loja_id ORDER BY nome ASC");
    ...
    if ($acao === 'adicionar' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($nome === '') {
            echo json_encode(['erro' => 'Nome da categoria obrigatório.']);
            exit;
        }

        // Evitar duplicidade no mesmo loja
        $chk = $pdo->prepare("SELECT COUNT(*) FROM categorias_produtos_lojas WHERE nome = :nome AND loja_id = :loja_id");
        $chk->execute([':nome'=>$nome, ':loja_id'=>$loja_id]);
        if ($chk->fetchColumn() > 0) {
            echo json_encode(['erro' => 'Já existe categoria com esse nome nesta loja.']);
            exit;
        }

        $stmt = $pdo->prepare("INSERT INTO categorias_produtos_lojas (nome, loja_id, ativo) VALUES (:nome, :loja_id, 1)");
        $stmt->execute([':nome'=>$nome, ':loja_id'=>$loja_id]);
        echo json_encode(['sucesso' => 'Categoria adicionada com sucesso.', 'id' => $pdo->lastInsertId()]);
        exit;
    }
    ...
    if ($acao === 'deletar' && isset($_REQUEST['id'])) {
        ...
        if ($chk->fetchColumn() > 0) {
            echo json_encode(['erro' => 'Não é possível excluir: existem produtos vinculados a esta categoria.']);
            exit;
        }

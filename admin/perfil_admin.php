...
// Se não encontrou admin, limpa sessão e redireciona
if (!$admin) {
    session_destroy();
...
// Se não encontrou loja, cria uma loja padrão e vincula ao admin
if (!$loja) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === 0) {
        ...
        $stmt->execute([
            ':foto' => 'uploads/' . $novo_nome,
            ':id' => $admin_id
        ]);
        $mensagem_foto = "Foto de perfil atualizada!";
        $admin['foto'] = 'uploads/' . $novo_nome;
    }
    ...
    if (isset($_POST['status_loja'])) {
        ...
        $stmt = $pdo->prepare("UPDATE lojas 
            SET status=:status, nome=:nome, endereco=:endereco, estado=:estado, cidade=:cidade, bairro=:bairro, 
                cep=:cep, logo=:logo, mensagem=:mensagem, horarios=:horarios 
            WHERE id=:id");
        $stmt->execute([
            ':status' => $status,
            ':nome' => $nome_loja,
  ...
  <div class="row justify-content-center">
    <div class="col-md-10">
      <div class="card shadow-sm p-4">
        ...
        <?php endif; ?>
        <?php if ($mensagem_foto): ?>
          <div class="alert alert-success"><?= htmlspecialchars($mensagem_foto) ?></div>
        <?php endif; ?>

        <!-- ADMIN -->
        <div class="text-center mb-4">
          <img src="<?= !empty($admin['foto']) ? '../'.htmlspecialchars($admin['foto']) : 'https://via.placeholder.com/150?text=Sem+Foto' ?>" 
               class="profile-img me-3" alt="Foto de perfil">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="nome_loja" class="form-control" value="<?= htmlspecialchars($loja['nome'] ?? '') ?>" required>
            </div>

            <div class="mb-3">
                <label>Endereço</label>
                <input type="text" name="endereco" class="form-control" value="<?= htmlspecialchars($loja['endereco'] ?? '') ?>">
            </div>

            <div class="row">
              <div class="col-md-4 mb-3">
                  <label>Estado</label>
                  <input type="text" name="estado" class="form-control" value="<?= htmlspecialchars($loja['estado'] ?? '') ?>">
              </div>
              <div class="col-md-4 mb-3">
                  <label>Cidade</label>
                  <input type="text" name="cidade" class="form-control" value="<?= htmlspecialchars($loja['cidade'] ?? '') ?>">
              </div>
              <div class="col-md-4 mb-3">
                  <label>Bairro</label>
                  <input type="text" name="bairro" class="form-control" value="<?= htmlspecialchars($loja['bairro'] ?? '') ?>">
              </div>
            </div>

            <div class="mb-3">
                <label>CEP</label>
                <input type="text" name="cep" class="form-control" value="<?= htmlspecialchars($loja['cep'] ?? '') ?>">
            </div>

            <div class="mb-3">
                <label>Mensagem Promocional</label>
                <input type="text" name="mensagem_promocional" class="form-control" 
                       value="<?= htmlspecialchars($loja['mensagem'] ?? '') ?>" 
                       placeholder="Entre cadastros e veja promoções imperdíveis...">
            </div>

            <div class="mb-3">
                <label>Logo da Loja</label><br>
                <?php if (!empty($loja['logo'])): ?>
                    <img src="<?= '../'.htmlspecialchars($loja['logo']) ?>" class="logo-img mb-2" alt="Logo da Loja"><br>
                <?php endif; ?>
            ...
            ?>
                <div class="row mb-2 align-items-center">
                  ...
                  <div class="col-md-5">
                    <input type="time" name="horarios[<?= $dia ?>][entrada]" class="form-control input-horario" value="<?= $entrada ?>">
                  </div>
                  <div class="col-md-5">
                    <input type="time" name="horarios[<?= $dia ?>][saida]" class="form-control input-horario" value="<?= $saida ?>">
                  </div>
                </div>
            <?php endforeach; ?>
